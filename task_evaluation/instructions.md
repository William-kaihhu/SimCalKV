...
The output length can affect the quality of the model’s generation results. For the same dataset, it is sufficient to use a consistent standard. You can adjust the values according to your hardware capacity.
...
## 3. Bias Vector Ablation Study

In **`analysis/query_test`**, two datasets are provided to compute the elements of the bias vector `delta_k`, which is used for comparison with `ln2`.
