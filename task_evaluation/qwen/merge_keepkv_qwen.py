...
def get_qt_qwen(layer_idx: int,
    ...
    Q = get_q_proj_from_attn(attn_module, hidden_states[layer_idx])
    b, s, _ = Q.shape
...
def divide_by_layer_qwen(layer_idx: int,
                         ...
                         attention,
                         ratio: float):
    K_matrix = key_value_cache[layer_idx][0]
    V_matrix = key_value_cache[layer_idx][1]

    K_of_head = K_matrix[0, head_idx, :, :]
    V_of_head = V_matrix[0, head_idx, :, :]
    ...
    if Ke_tokens_keys.size(0) == 0 or Kc_tokens_keys.size(0) == 0:
        ...
        sim_matrix = Ke_norm @ Kc_norm.transpose(-2, -1)
...
def merge_Ve_to_Vc_qwen(model,
    ...
    for i in range(Ve_num):
        ...
        Vc_new[tgt] = merge_by_token(Ve[i], Vc_new[tgt], w_e, w_c)
