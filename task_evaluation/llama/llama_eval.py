...
def evaluate_example(model, tokenizer, prompt, reference, method="SimCalKV", compress_ratio=0.1, device="mps"):
    ...
    for i in range(1):  # Inference
        ...
        total_kv, generated_tokens, durations_cached_s = generate_text(model, tokenizer, 128, {
        ...
        total_kv_merged, generated_tokens_merged, durations_cached_s_merged = generate_text(model, tokenizer, 128, {
            "input_ids": next_token_id.reshape((1, 1)),
            "attention_mask": torch.cat(
                [inputs["attention_mask"], torch.tensor([[1]], device=device)], dim=1
            ),
        ...
        memory_after = measure_kv_memory(total_kv_merged) * compress_ratio
